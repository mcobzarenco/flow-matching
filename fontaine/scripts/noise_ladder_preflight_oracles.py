...
Checks (all abort, never silent):
  ...
  (iv)  npz + report provenance: routed run carries the m64 bank sha
        AND ticket_map_sha256 == the pre-registered map sha
        `15d9293553ac1a88…` at sample_draws 1; the plain run carries
        the t2-only bank's file sha and NO map;
  (v)   bank lineage: the t2-only bank byte-equals m64[2:3];
  (vi)  map coverage (CPU): the committed map loads, reproduces the
        pre-registered sha, covers EVERY dataset of the full panel
        plan, and its image is within top-10 + {33};
...
def _pred_key(npz: np.lib.npyio.NpzFile, path: str) -> str:
    keys = [k for k in npz.files if k.startswith("pred:")]
    if len(keys) != 1:
        _fail(f"{path}: expected exactly one pred: key, got {keys}")
    return keys[0]
...
def adjudicate(
    ...
    # (i) identity — matched composition.
    ...
    if str(routed["ticket_map_sha256"]) != map_sha:
        _fail(
            f"routed npz map sha {routed['ticket_map_sha256']} != committed {map_sha}",
        )
    ...
    if routed_report.get("ticket_map_sha256") != map_sha:
        _fail(
            f"routed report map sha {routed_report.get('ticket_map_sha256')} != {map_sha}",
        )
    ...
    # (vi) map coverage on the FULL panel plan.
    mapping, sha = load_ticket_map(Path(analysis_path), m64_bank.shape[0])
    if sha != map_sha:
    ...
    uncovered = sorted(panel_datasets - set(mapping))
    if uncovered:
        _fail(f"map misses {len(uncovered)} panel dataset(s), first {uncovered[:5]}")
    if not set(mapping.values()) <= set(TOP10):
        _fail(f"map image {sorted(set(mapping.values()))} escapes top-10 + {{33}}")
    ...
    for repo in PREFLIGHT_DATASETS:
        if mapping[repo] != PREFLIGHT_TICKET:
            _fail(f"{repo} routes to {mapping[repo]}, expected {PREFLIGHT_TICKET}")
...
def _selftest() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_str:
        ...
        print("selftest 4 RED worlds: all abort as required")
        print("SELFTEST PASS")
