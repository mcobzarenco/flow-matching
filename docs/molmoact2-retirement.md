...
4. **Objective matrix** on molmoact2-format checkpoints:
   ...
   `joint_ce` slot. Joint loss = fixed-weight 1.0 sum (the KI
   no-tuning precedent; re-weighting is a registered amendment, not a
   knob). **PENDING owner bless of the fixed weight.**
...
7. **Gate-d disposition** (§8.13's remaining gate: the rig-rung
   fine-tune repeat through `bijou.train`, ≤6 GPU-h): **PENDING owner
   call** — run it in phase 0's GPU window (recommended; it doubles
   as the joint path's smoke) or waive on the recorded evidence
   (2-step corridor + E2E byte-parity + fontaine's independent line).
...
our collator, not the port). GPU window: gate-d per decision 7.
...
| rig-rung class reproduction | 0 or waived | gate-d (decision 7) |

## 7. Open decisions before implementation starts

1. Decision 4's fixed joint weight (recommended: 1.0, amendable).
2. Decision 7: run gate-d in phase 0 vs waive on record.
