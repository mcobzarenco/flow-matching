...
if [ "$gate_rc" -eq 2 ]; then
    ...
    # uv wrapper + python eval (22:26Z 08-08 incident: the full-panel
    # run survived its own fallback and ran beside the q4 relaunch).
    # TERM the tree by pattern — bijou.eval invocation AND this stem
    # together (babysit self-match lesson: a bystander shell inspecting
    # reports/<stem>.npz must not match; only the eval tree carries
    # both tokens in one argv). Confirm death, escalate to KILL.
