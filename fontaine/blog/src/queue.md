...
**Depth call:** depth 2 open at 03:5xZ 08-12: sim-visual-matching (cpu, THE lever, baseline now measured by the closed OOD probe) + sim100-v1-rerun (gpu-local successor, pends the matching landing, probe re-read as go/no-go gate); sim-encoder-ood-probe CLOSED this session; molmo-flow lane owner-side; rig-mixture-screen-exec owner_hold.

**11 open** (Live 0 · Queued 2 · Blocked 9 · Done 120)
...
**boundary:** Queued 03:4xZ 08-12 at the OOD-probe close. Executable only after sim-visual-matching lands its so101_sim.py visual deltas; the probe re-read (~0.02 GPU-h) is the cheap go/no-go gate before the ~2-4 GPU-h eval. · [pre-reg](posts/2026-08-11-prereg-sim-policy-eval-100seeds.md)
