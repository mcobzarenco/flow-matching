#!/usr/bin/env bash
# gripfix endpoint battery — GPU legs (queue: gripfix-endpoint-close;
# pre-reg posts/2026-08-21-prereg-clean-gripper-carrier.md, grid
# frozen: >=20 carrier / <=10 not-sole-carrier / 11-19 ambiguous).
#
# Protocol copied EXACTLY from launch_democlean_endpoint_battery.sh
# (itself the onerig protocol; the paired reads vs democlean 8/100 —
...
#   systemd-run --user --unit=fontaine-gripfix-endpoint-battery \
...
#     bash ~/flow-matching/fontaine/scripts/launch_gripfix_endpoint_battery.sh
set -euo pipefail
...
RUN=grasp_sft_v2_joint_1gpu_pdnorm_gripfix
CKPT="$HOME/checkpoints/finetune/$RUN/step_003000"
OUT=outputs/sim/grasp_sft/gripfix_endpoint
...
MUJOCO_GL=egl uv run python -m sim.rollout_sim \
    ...
    2>&1 | tee /home/ubuntu/eval__gripfix_endpoint_sim100.log
