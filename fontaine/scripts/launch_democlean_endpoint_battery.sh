#!/usr/bin/env bash
# democlean endpoint battery — GPU legs, chained (queue:
# democlean-endpoint-close; pre-reg
# posts/2026-08-20-prereg-demos-plus-clean.md).
#
# Protocol copied EXACTLY from launch_onerig_endpoint_battery.sh (the
# paired reads vs onerig / control / convicted are only valid
...
#   systemd-run --user --unit=fontaine-democlean-endpoint-battery \
...
#     bash ~/flow-matching/fontaine/scripts/launch_democlean_endpoint_battery.sh
set -euo pipefail
...
RUN=grasp_sft_v2_joint_1gpu_pdnorm_democlean
CKPT="$HOME/checkpoints/finetune/$RUN/step_003000"
OUT=outputs/sim/grasp_sft/democlean_endpoint
...
MUJOCO_GL=egl uv run python -m sim.rollout_sim \
    ...
    2>&1 | tee /home/ubuntu/eval__democlean_endpoint_sim100.log
