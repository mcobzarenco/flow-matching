#!/usr/bin/env bash
# onerig endpoint battery — GPU legs, chained (queue: onerig-endpoint-close;
# pre-reg posts/2026-08-19-prereg-demos-plus-one-rig.md).
#
# Protocol copied EXACTLY from launch_pdnorm_endpoint_battery.sh (the
# paired reads vs the control and the convicted cell are only valid
...
#   systemd-run --user --unit=fontaine-onerig-endpoint-battery \
...
#     bash ~/flow-matching/fontaine/scripts/launch_onerig_endpoint_battery.sh
set -euo pipefail
...
RUN=grasp_sft_v2_joint_1gpu_pdnorm_onerig
CKPT="$HOME/checkpoints/finetune/$RUN/step_003000"
OUT=outputs/sim/grasp_sft/onerig_endpoint
...
MUJOCO_GL=egl uv run python -m sim.rollout_sim \
    ...
    2>&1 | tee /home/ubuntu/eval__onerig_endpoint_sim100.log
