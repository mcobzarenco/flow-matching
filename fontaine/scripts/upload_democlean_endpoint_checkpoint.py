"""Upload the demos+clean endpoint (step 3000) to fontaine-checkpoints.
...
1.0 --insulate-flow --per-dataset-flow-norm --recompute-stats,
--init-from the owner's re-converted molmoact2-so101-released; mix =
grasp_demos_v2/merged + so101_pick_place_clean (x4) ONLY (v2 dropped —
the poison-pinning cell: the convicted mix minus v2, equivalently the
other single-rig-dataset cell vs onerig); 3000 steps eff-96 on the
local H100, unit fontaine-v2-joint-pdnorm-democlean launched
14:15:41Z 2026-08-20; pre-reg
posts/2026-08-20-prereg-demos-plus-clean.md. Banked at the endpoint
verdict (frozen-grid sim100 read) per the standing banked-checkpoints
rule — bankable on either decisive branch (poison exhibit or second
exoneration cell).
"""
...
RUN_DIR = Path.home() / "checkpoints/finetune/grasp_sft_v2_joint_1gpu_pdnorm_democlean"
STEP_DIR = RUN_DIR / "step_003000"
DEST = "grasp_sft_v2_joint_pdnorm_democlean_step3000"
