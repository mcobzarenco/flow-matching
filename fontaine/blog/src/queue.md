...
**14 open** (Live 0 · Queued 2 · Blocked 12 · Done 145)
...
**boundary:** Item 4 (loop harness) + phase-2 run pre-reg finalization are the critical path (owner delegation 11:07/11:18Z: no confirmation waits — proceed by frozen rules). The RUN launches only on the finalized pre-reg: memo §5 ladder re-priced at the measured ~0.4 s/chunk batch-1 pace + ~1.4 s/round batched (8 workers, arm-A measured), trainable surface per memo §4 (option B recommended, owner veto window passed unanswered), reward per memo §3. Rollout+replay halves are DONE — the loop harness is the last instrument between here and the launch.
