"""Results chart for the arm micro-texture read (queue
`sim-arm-texture-followup`; eval-report dark scheme, hues follow the
...
def main() -> None:
    ...
    ap.add_argument(
        "--analysis",
        type=Path,
        default=Path("reports/analysis__sim_arm_texture_read.json"),
    )
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("outputs/sim/arm_texture/chart__arm_texture_read.png"),
    )
    args = ap.parse_args()
    ...
    rows = [
        ...
        (
            "links on plate\nonly_links_photo -> only_links_tex",
    ...
    ax.set_title(
        "Micro-texture on top of the photometric grade",
        color=TEXT,
        fontsize=11,
        loc="left",
    )
    ...
    reads = [
        ("PRIMARY\nv3_tex - v3_photo", results["primary_v3_tex_vs_v3_photo"], FRAME),
        (
            "MECHANISM\nonly_links_tex - only_links_photo",
            results["mechanism_only_links_tex_vs_only_links_photo"],
            LINKS,
        ),
    ]
    spans = []
