...
**boundary:** Queued 07:0xZ 08-12 at the v1 close (rides the v1 pre-reg's instrument + axes; wrist was explicitly secondary there). Executable now; independent of the inpainting item. · [pre-reg](posts/2026-08-12-prereg-sim-visual-matching.md)
...
**boundary:** Queued 06:5xZ 08-12 at the v1 close. Executable now (CPU + ~0.02 GPU-h probe reads); pends nothing. If the owner instead calls the behavioral spot-check on sim100-v1-rerun, run that first - it may show the geometry fixes already changed behavior, re-scoping what inpainting must buy.
