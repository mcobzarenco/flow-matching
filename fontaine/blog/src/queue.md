...
**Updated:** 2026-08-11T06:15:00Z

**Depth call:** depth 2 open at 06:1xZ 08-11: molmoact2-firstclass-port (items 1+2+3 CLOSED; item 3 closed this session 06:1xZ: G2 amended-PASS both directions + G3 oracled, Amendment 1 in the pre-reg; item 4 = AE fine-tune in OUR trainer, <=6 GPU-h gate, opens next port session) + er60k-endpoint-postprocess (endpoint @60000 ~12:3xZ 08-11, chained panel_v2 rides in-unit; the postprocess session owns the paired reads). er-60k LIVE box to endpoint. Lit refill lane owner-paused (00:23Z 08-10).

**9 open** (Live 1 · Queued 2 · Blocked 6 · Done 105)
...
**boundary:** Opens after molmoact2-rig-ft-postprocess (endpoint read is time-sensitive); ~3-4 focused sessions, items in order 1-&gt;2-&gt;3-&gt;4, each with its own parity/oracle gate before the next. PRE-REG POSTED 21:0xZ 08-10 (parity gates G1-G4 frozen: G1 AE-module bf16 forward parity, G2 end-to-end 240-row anchor reproduction 28.9454/3.2301, G3 check.py oracle harness, G4 our-trainer rung-1 repro &lt;=6 GPU-h; port GPU total &lt;=8). ITEM 1 MODULE HALF DONE 22:4xZ 08-10 (ed3f6e8): bijou/molmoact2/action_expert.py + load_action_expert_state + 9 CPU oracles in check.py; G1 CLOSED BOTH RUNGS — byte-identical (max|delta| 0.0e+00) vs their HF remote-code ActionExpert on real step2000 weights, 3 seeds each, CPU/fp32 AND cuda/bf16 (fontaine/scripts/molmoact2_ae_parity.py). Config measured off export: h768/36 blocks/8 heads/mlp3072, 577,564,448 params exact; llm_kv_dim = head_dim 128 x 8 kv heads = 1024. Item-2 finding: their HF inference expert rejects continuous state_embeddings (state enters as prompt tokens; state_encoder is train-only identity-injected). ITEM 1 FULLY CLOSED 23:4xZ 08-10 (wiring commit after ed3f6e8): bijou/molmoact2/wiring.py (extract_kv_states off Molmo2KVCache per-layer post-RoPE -&gt; [B,S,1024] w/ 36:36 hard check + seq_len truncation; continuous-mode encoder_attention_mask; ascending-Euler generate_actions fp32 t-grid w/ padded-dim masking init/v/x; validate_inference_config loud guards on action_mode=both + depth gate) + action_expert._time_conditioning (their HF inference sinusoid-fp32-then-cast semantics; no-op at uniform dtype, G1 re-run 0.0). WIRING PARITY byte-identical (max|d| 0.0e+00) vs their shipped MolmoAct2Model action path (unbound on stub w/ real AE weights): kv_extraction + both mask branches + FULL 10-step flow loop, 3 seeds, CPU/fp32 AND cuda/bf16 (fontaine/scripts/molmoact2_wiring_parity.py). +20 CPU oracles, check.py 628 green. REMAINING: items 2-4 (item 2 opens next port session). WIRING NOTES (read off their HF modeling + config 22:4xZ, next-session contract): (1) KV extraction = per-layer (K,V) off the prompt-forward cache, truncated to prompt seq_len, (b,h,s,d)-&gt;(b,s,1024) via _cache_to_sequence; 36 LLM layers -&gt; 36 AE blocks 1:1 (hard check). (2) encoder attention mask = attention_mask minus EOS; discrete-output-span masking only under action_mode='both' — SO100_101 is action_mode='continuous', so NOT needed for rig path. (3) depth gate OFF in this checkpoint (action_expert_depth_gate=false) — skip for rig path, leave a loud config guard. (4) inference flow loop = plain ascending Euler: x~N(0,1) in AE dtype, t_idx = idx/steps fp32, x += v/steps, 10 steps; mask_action_dim_padding=true zeroes padded action dims on BOTH v and x each step (dims beyond the 6 real joints of 32). flow_matching_time_offset/scale/beta_* are TRAIN-side timestep-sampling params, not used in the HF inference loop. (5) state is DISCRETE (state_format=discrete, 256 state tokens, n_obs_steps=1) — enters via prompt tokens, item-2 territory; AE state_embeddings path unused at inference. ITEM 2 CLOSED 01:3xZ 08-11 (71e146b): bijou/molmoact2/processing.py — q01/q99 normalize+clamp (state in) / clamp+unnormalize (action out), exact lerobot QUANTILES formula (eps 1e-8, torch f32 op order); task-text normalization (prefix/quote strip, sentence join, lower); 256-bin discrete state string (np.rint half-to-even, nan-&gt;0 inf-&gt;+-1); robot prompt template + chat wrap + &lt;action_output&gt;; resize-mode image path (their shipped crop_mode=resize: ONE 378x378 view/image, grid (14,14,0,0), 196 pooled tokens, NO col tokens) with uint8-end-to-end semantics (pack coerces to uint8 BEFORE resize, resize rounds back to uint8 — float-path resize NOT equivalent); tokenization + BOS insert (bos==eos==&lt;|im_end|&gt; 151645, pad &lt;|endoftext|&gt; 151643); infer_max_sequence_length loud guard. TOKEN-ID DELTA PINNED: molmoact2 tokenizer re-homes image specials (im_start/end/patch/col/lowres = 155648-155652) and inserts state/action/setup/control vocab at 151669-151934 (&lt;state_0&gt;=151675 contiguous, &lt;action_output&gt;=151931) — NOT molmo2 ids; verified vs the loaded tokenizer in oracles. PARITY: goldens banked from THEIR real lerobot pipeline run in their venv (fontaine/scripts/molmoact2_processing_goldens.py, 9 input + 3 action cases: mixed sizes/dtypes/cam counts, state clamp/nan/inf edges, task string edge cases; 108K fixtures, bank_processor_goldens precedent) — reproduced BYTE-EXACT (max|d| 0.0) on input_ids, pixel values (strided samples + per-crop stats), pooling idx, normalized state, unnormalized actions; uint8 resize bit-identical across torchvision 0.25/0.26. +23 CPU oracles tests/test_molmoact2_processing.py, check.py 651 green. ITEM-3 CONTRACT (pinned 01:4xZ): (a) backbone injection already parameterized — their config.json carries image_patch_id=155650 and our Molmo2 config loader reads image_patch_id from config (no code change); (b) their modeling runs bidirectional attention where token_type_ids==1 (processor marks IMAGE_TOKENS membership with the NEW id set) — bijou/encoders/molmo2.py IMAGE_TYPE_IDS is the OLD 151936+ molmo2 set, the item-3 harness must supply the molmoact2 set (resize mode emits only im_start/im_patch/im_end); (c) G2 anchors = 240-row zero-shot 28.9454 / state-copy 3.2301 on the step2000 rig-ft export; assembly = bijou.molmo2 backbone (their trunk weights) + item-1 wiring/AE + item-2 pack_action_example. ITEM 3 CLOSED 06:1xZ 08-11 (9c15647): bijou/molmoact2/predictor.py = first-class MolmoAct2Predictor (item-2 pack -&gt; bijou.molmo2 trunk forward w/ retained KV -&gt; item-1 wiring/expert -&gt; their exact output tail incl. bf16 round-trip); loaders accept molmoact2 checkpoints (model_type, AE-key skip, persisted rope inv_freq skip); fontaine/scripts/molmoact2_e2e_parity.py = G2 read on the 240 banked anchor rows, same per-row seeds. G2 AMENDED-PASS both directions: released pooled |d| 0.0410 (inside the original 0.05) anchor 28.9456 vs 28.9454; rig-ft step2000 pooled |d| 0.0541 anchor 3.2321 vs 3.2301 -&gt; Amendment 1 posted in the pre-reg (budget 0.075 priced off the measured floor; localization: both stacks byte-deterministic incl. their pipeline re-run 240/240 byte-identical, inputs byte-identical, their-KV through OUR flow loop reproduces banked to 0.0000, residual = 1-ulp bf16 kernel-order rounding in the vision tower). G3 CLOSED: 8 CPU oracles tests/test_molmoact2_predictor.py (tiny wide-vocab trunk wearing the real token layout), check.py 659 green. SCOPE CORRECTIONS BANKED: (a) released SO100_101 is action_mode='both' NOT 'continuous' (item-1 note wrong) — 'both' encoder mask (EOS strip incl. BOS-is-im_end + discrete-span pairing) implemented + oracled in wiring.py; (b) image special ids are CHECKPOINT-DEPENDENT (released 154624+ no depth vocab, rig-ft exports 155648+ with 128 depth tokens) — predictor resolves ids per checkpoint via tokenizer, processing.py pinned constants match the rig-ft/training layout only. ~0.7 GPU-h this session, port total well under the 8 gate. REMAINING: item 4 only (G4: our-trainer AE fine-tune, matched recipe, beats both anchors + loss corridor, &lt;=6 GPU-h). · [pre-reg](posts/2026-08-10-prereg-molmoact2-firstclass-port.md)
