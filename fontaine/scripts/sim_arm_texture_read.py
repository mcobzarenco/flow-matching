"""Arm micro-texture — the registered 20x5 probe gate read (queue
`sim-arm-texture-followup`; pre-reg posted before this runs).

Two production v3 instances over the SAME (seed, draw) schedule — the
arm_photometrics='v1' grade alone and the grade + arm_texture='v1'
composite-stage micro-texture — hooked at ``_composite`` exactly like
the photometric read, so every arm shares physics/plate/lighting/noise
per slot and the paired delta IS the texture's visible-pixel effect:
  ...
  patched instance:  v3_tex, only_links_tex

In-run oracles: hooked v3_photo/v3_tex bit-exact vs their own production
observation; the two instances' settled qpos bit-equal per slot (the
texture consumes no shared-RNG draws); frames bit-equal outside the
dilated arm-class mask (the texture is arm-local).
...
paired dknn5 CI95 (10k resamples, rng 0) of v3_tex vs v3_photo entirely
below 0 (toward real). MECHANISM: same for only_links_tex vs
only_links_photo. Anchors: plate_only, banked v3 0.713, banked
only_links_photo 0.652, real_fg 0.328.

Usage:
  MUJOCO_GL=egl uv run python fontaine/scripts/sim_arm_texture_read.py \
      --out reports/analysis__sim_arm_texture_read.json
"""
...
def main() -> int:
    ...
    print("patched pass (grade + texture): 2 arms x 100 slots ...")
    patched = SO101Sim(
        render_style="v3",
        post_backend="numpy",
        arm_photometrics="v1",
    ...
    patched_arms, _ = hook(
        patched,
        {"v3_tex": full, "only_links_tex": links, "__armclass__": armclass},
    )
    for index, slot in enumerate(slots):
        appearance = 1000 * slot["draw"] + slot["seed"]
        obs_patch = patched.reset(slot["seed"], appearance_seed=appearance)
        if not np.array_equal(obs_patch.top, patched_arms["v3_tex"][-1]):
        ...
        if not np.array_equal(patched.data.qpos, base_qpos[index]):
            raise SystemExit(
                ...
                "the texture must consume no shared-RNG draws",
            )
        outside = outside_masks[index]
        if not np.array_equal(
            base_arms["v3_photo"][index][outside],
            obs_patch.top[outside],
        ):
            raise SystemExit(
                f"slot {slot}: frames differ outside the dilated arm "
                "mask — the texture must be arm-local",
            )
    ...
    primary = paired_read(scores["v3_tex"], scores["v3_photo"])
    mechanism = paired_read(scores["only_links_tex"], scores["only_links_photo"])
    results = {
        "real_heldout": {"mean": float(held.mean()), "std": float(held.std())},
        "clean_anchor": clean_read,
        "arms": arms_read,
        "primary_v3_tex_vs_v3_photo": primary,
        "mechanism_only_links_tex_vs_only_links_photo": mechanism,
        "primary_pass": bool(primary["ci95"][1] < 0),
        "mechanism_pass": bool(mechanism["ci95"][1] < 0),
        "context": {
            "only_links_tex_vs_plate_only": paired_read(
                scores["only_links_tex"],
                scores["plate_only"],
            ),
    ...
    payload = {
        "config": {
            ...
            "per-slot qpos/outside-arm bit-equality oracles; noise-RNG state "
            "restored per arm",
            "texture": SO101Sim.ARM_TEXTURE_V1,
            "slots": slots,
            "commit": commit,
        },
        "results": results,
    ...
    print(f"PRIMARY v3_tex vs v3_photo: {primary}")
    print(f"MECHANISM only_links_tex vs only_links_photo: {mechanism}")
