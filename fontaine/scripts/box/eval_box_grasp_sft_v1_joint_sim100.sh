#!/usr/bin/env bash
# grasp_sft_v1_joint endpoint sim100 — 8xA100 box, seed-sharded.
# (queue item grasp-sft-v1-endpoint-boundary; anchors: route-C joint
# probe 44/100 unseen flow, token leg vs the R2 bar >=20/100.)
#
...
#
# Usage (on the box):
#   bash eval_box_grasp_sft_v1_joint_sim100.sh smoke   # 2 seeds/leg on gpu0/gpu4, ~3 min
#   bash eval_box_grasp_sft_v1_joint_sim100.sh full    # flow gpu0-3, token gpu4-7
...
MODE="${1:?usage: eval_box_grasp_sft_v1_joint_sim100.sh smoke|full}"
CKPT=~/checkpoints/finetune/grasp_sft_v1_joint_8xa100/step_003000
OUT=outputs/sim/grasp_sft/v1_endpoint
mkdir -p "$OUT"
...
run_shard() { # gpu leg seed n extra-args...
  ...
  CUDA_VISIBLE_DEVICES="$gpu" MUJOCO_GL=egl uv run python -m sim.rollout_sim \
      ...
      > "/home/ubuntu/eval__sft_v1_${leg}_s${seed}.log" 2>&1
}
...
for i in "${!pids[@]}"; do
  if wait "${pids[$i]}"; then
    ...
    echo "shard ${names[$i]}: FAILED (log /home/ubuntu/eval__sft_v1_${names[$i]}.log)"
    rc=1
  fi
done
