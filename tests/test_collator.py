...
def test_codecless_action_table_is_refused() -> None:
    """A merged action table without a codec has nothing to tokenize —
    decode-side tables are the family's quantile table, never collator
    state (batch stats never carry a table into a decode), so the
    construction is refused loudly."""
    with pytest.raises(ValueError, match="nothing to tokenize"):
        collator(
            action_q01=torch.full((DIM,), -3.0),
            action_q99=torch.full((DIM,), 5.0),
        )
